...
then
  ...
  then
    ...
    while [[ "$(curl -k -s -o /dev/null -w ''%{http_code}'' https://kong-admin.k3s.jack.local)" != "401" ]]; do printf .; sleep 2; done
    echo ''

    # apply configuration
    echo 'Applying initial Kong deck configuration'
    deck --tls-skip-verify --kong-addr="https://kong-admin.k3s.jack.local" --headers="kong-admin-token:K1ngK0ng" sync -s initial-kong.yaml
  fi
fi
